...
//        System.out.println("Output file: " + ((ConvertQuery)q).outFileName);
        ...
        if(!commited){
          if(q.queryType.contains("CONVERT"))
        {
            String testname = ((ConvertQuery)q).xmlFileName + ".xml";
            String filename = ((ConvertQuery)q).outFileName + ".txt";
            String xsdname = ((ConvertQuery)q).xsdFileName + ".xsd";
            
            if(xsdname.contains("null"))
            {
                xsdname = "";
            }
            
            
            Document dd = Database.convert(testname, xsdname, filename);
        }
          else if(q.queryType.contains("INSERT")){
             ArrayList<String> fieldnames = ((InsertQuery)q).fields;
             ArrayList<String> invalues = ((InsertQuery)q).values;
             
              
              MainMenu.DOM = db.insert(q.tablename, fieldnames, invalues, MainMenu.DOM);
              
          }
          else if(q.queryType.contains("INPUT")){
              System.out.println(q.queryType + ((InputQuery)q).txtFileName);
              
              db.input(((InputQuery)q).txtFileName);
          }
          else if(q.queryType == "SELECT"){
              System.out.println(q.queryType + "hererererer");
            db.selectW(MainMenu.DOM,q.tablename);
            /*public static void selectW(SelectQuery q, Document DOM){
    
                System.out.println("sdfsf" + q.columns.get(0));
                }*/
//              System.out.println("I;m here");
//              if(ast){
//                  if(!where){
//                  db.selectW((SelectQuery)q, MainMenu.DOM);
//                  }
//                  else{
//                      
//                  }
//              }
//               else if(((SelectQuery)q).columns.size() != 0){
//                   if(!where){
//                  db.selectW((SelectQuery)q, MainMenu.DOM);
//                   }
//                   else{
...
//              }
            ...
            //MainMenu.DOM = db.selectA(MainMenu.DOM, q.tablename)
          }
          else if(q.queryType == "TSELECT"){
              //FOR tSELECT * FROM Tablename
             // System.out.println("hererererer");
              db.selectT(MainMenu.DOM, q.tablename);
//              public static void selectT(SelectQuery q, Document DOM){
//    
//                System.out.println("sdfsf" + q.columns.get(0));
//                }
          }
          else if(q.queryType == "DELETE"){
              db.delete(q.tablename, MainMenu.DOM);
          }
        }
    ...
    public static void command()    {
        ...
        else if(tokens[currentToken].type.toString().contains("COMMIT")){
            db.commit(MainMenu.DOM);
            commited = true;
        }
        else if(tokens[currentToken].type.toString().contains("INPUT"))
        {
            q = new InputQuery();
            currentToken++;
            if(tokens[currentToken].type.toString().contains("ID")) {
                String fn = tokens[currentToken].data;
                currentToken++;
                if(tokens[currentToken].type.toString().contains("DOT")) {
                    ...
                    if(tokens[currentToken].type.toString().contains("TXT")) {
                        ...
                        ((InputQuery)q).assignInputFile(fn);
                        currentToken++;
                    }
                    else rejected("ERROR: Invalid TXT file type");
                }
                else rejected("ERROR: Invalid TXT file");
            }
    ...
    public static void outputFile() {
        System.out.println(tokens[currentToken].data + "whatt");
        if(tokens[currentToken].type.toString().contains("ID")) {
            currentToken++;
            if(tokens[currentToken].type.toString().contains("DOT")) {
                currentToken++;
