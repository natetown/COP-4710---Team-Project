...
//         printTokens(); // test that the two methods are correct by printing their contents
         ...
         if(stmt(tokens[P]) == true && error == false){ // statement is correct
            ...
            if(tokens[0] == "CREATE"){
                ...
                {
                   //creates database
                   MainMenu.DOM = db.createDatabase(values[2]);   
                }
                else if(tokens[1].equals("TABLE"))
                {
                   MainMenu.DOM = db.createTable(values[2], MainMenu.DOM);
                ...
                if(tokens[1].equals("DATABASE")){
                //Call drop function from database.java
                db.dropDatabase(values[2]);
                }
                else if(tokens[1].equals("TABLE")){
                    db.dropTable(values[2], MainMenu.DOM);
                }
            }
            else if(tokens[0].equals("SAVE")){
                //calls save database function
               db.saveDatabase(MainMenu.DOM);
               
            }
            else if(tokens[0].equals("LOAD")){
               
                MainMenu.DOM = db.loadDatabase(values[2]);
